...
class Sock{
    ...
    public function __construct($address, $port){
        ...
        $this->master=$this->WebSocket($address, $port);//创建socket并把保存socket资源在$this->master

        $this->sockets=array($this->master);//创建socket连接池
    }
    ...
    */
        ...
        while(true)//死循环，直到socket断开
        {
            ...
            */
            ...
            foreach($changes as $sock){
                ...
                if($sock==$this->master){//如果有新的client连接进来

                    $client=socket_accept($this->master);//接受一个socket连接

                    $key=uniqid();//给新连接进来的socket一个唯一的ID
                    ...
                    do{//读取该socket的信息，注意：第二个参数是引用传参即接收数据，第三个参数是接收数据的长度
                        $l=socket_recv($sock,$buf,1000,0);
                        $len+=$l;
                        $buffer.=$buf;
                    }while($l==1000);

                    $k=$this->search($sock);//根据socket在user池里面查找相应的$k,即健ID

                    if($len<7){//如果接收的信息长度小于7，则该client的socket为断开连接

                        $this->send2($k);//给该client的socket进行断开操作，并在$this->sockets和$this->users里面进行删除
                        continue;
                    }

                    if(!$this->users[$k]['shou'])//判断该socket是否已经握手
                    {
                        $this->woshou($k,$buffer);//如果没有握手，则进行握手处理
                    }else{
    ...
    function WebSocket($address,$port){
        ...
        $this->e('Listening'.$address.' port '.$port);
        return $server;
}
